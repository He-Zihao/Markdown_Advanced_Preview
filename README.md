...
first use. If the user settings file is deleted, the plugin recreates it from
...
GitLab parser uses these bundled local assets for GitLab API output:

1. KaTeX 0.18.1: `css/katex.min.css` and `js/katex.min.js`;
2. Mermaid 11.16.1: `js/mermaid.min.js`;
3. the integration script `js/gitlab_config.js`, loaded after both engines.

These are also the explicit `gitlab` entries in `settings_default.json`; no
KaTeX or Mermaid CDN is required by the default GitLab asset configuration.
...
Local CSS is embedded into generated HTML. Local JavaScript is referenced with
an absolute cross-platform `file://` URI, while remote URLs remain external.
Keeping JavaScript external prevents large bundles from appearing as inline
code in the HTML and lets MathJax 4 determine its resource root from
`document.currentScript.src`. The generated document therefore depends on the
configured local JavaScript files remaining at those paths. A same-name CSS
file beside the source (for example, `notes.css` beside `notes.md`) is appended
when `allow_css_overrides` is true.
